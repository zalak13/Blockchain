...
public class BasicTests extends TestCase {
    ...
    public List<NodeFactory> BASIC_MALICIOUS_NODE_FACTORIES = Arrays.asList(MaliciousNode0.FACTORY, MaliciousNode1.FACTORY,MaliciousNode3.FACTORY,MaliciousNode_Zalak_13.FACTORY,MaliciousNode_Zalak_23.FACTORY);
            ...
            for (NodeFactory mnf : BASIC_MALICIOUS_NODE_FACTORIES) {
                for (String graphFile : BASIC_GRAPH_FILES) {
                    for (String txFile : BASIC_TX_FILES) {
                        Scenario scenario = ScenarioParser.parse(cnf, mnf, "src/hw4/files/" + graphFile + ".txt", "src/hw4/files/" + txFile + ".txt", 10);
